...
public final class VisualLogger {
	...
	private void log(String message, Object[] params, Level level, Formatting formatting) {
		...
		if (FabricLoader.getInstance().isDevelopmentEnvironment()) {
			var text = Text.of(clazz.getSimpleName() + '/').copy();
			text.append(Text.of(level.name()).copy().formatted(formatting));
			text.append(Text.of(": " + ParameterizedMessage.format(message, params)));
